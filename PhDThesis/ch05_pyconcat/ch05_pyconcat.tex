...
\begin{figure}
	...
	\end{center}
	\caption[]{Hyperplane division (left) and binary tree (right) arrangement of 2-dimensional dataset with a \textit{k}-D Tree of depth 4. The origin is the top left and the size of the grid is 128x128 units. Image from Linköping University\protect\footnotemark.}
...
Unlike other investigators \citep{Coleman2010} we don't focus on complex post-selection transformations, but if there is a large discrepancy between the lengths of the target units and the corpus units or its pitch needs to be adjusted, some time compression or pitch-shifting can be applied, which we facilitate through the a transformation stage utilising the Rubber Band Library\footnote{\url{http://www.breakfastquay.com/rubberband/}} which is also used in the systems of \cite{Davies2013} and \cite{Smith2015}.
...
\begin{figure}
	...
	\end{center}
	\caption[MFCC plots of 3 different \textit{k}-Best Sequences]{MFCC plots of 3 different \textit{k}-Best Sequences}
